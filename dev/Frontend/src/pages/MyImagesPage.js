...
const MyImagesPage = () => {
  ...
  // State to remember the format of the row being edited
  const [originalEditParts, setOriginalEditParts] = useState([]);
  ...
  const updateImageOnServer = async () => {
    ...
    try {
      ...
      // console.log("Updating image with payload:", payload);
      await axios.patch(
    ...
    <div className="mi-bg">
      {loading && (
        <div className="mi-modal-overlay" style={{ zIndex: 9999 }}>
          <div className="mi-spinner-modal"><div className="mi-spinner" /><p className="mi-loading">Loading…</p></div>
        </div>
      )}

      {!selectedImage && (
        <img src={`${process.env.PUBLIC_URL}/images/HomeButton.png`} alt="Home" className="mi-home-btn" onClick={() => navigate("/main-page")} />
      )}
      ...
      <div className="mi-container">
        ...
        <div className="filters-card">
          <div className="filter-grid">
            <input className="mi-filter-input" placeholder="Name" value={filterName} onChange={(e) => setFilterName(e.target.value)} />
            <input className="mi-filter-input" placeholder="Description" value={filterDescription} onChange={(e) => setFilterDescription(e.target.value)} />
          </div>
          <div className="search-button-container">
            <button className="search-button" onClick={() => setCriteria({ name: filterName, description: filterDescription, page: 0 })}>Search</button>
          </div>
        </div>

        <div className="mi-images-section">
          {Object.keys(imagesMap).length === 0 ? (<p>No images available.</p>) : (
            Object.entries(imagesMap).map(([id, img]) => (
              <div key={id} className="tooltip">
                <div className="mi-image-item" onClick={() => { setSelectedImage(img); setSelectedImageId(id); }}>
                  <div className="mi-thumbnail-text"><h4>{img.name}</h4></div>
                </div>
                {img.description && (<div className="tooltip-bubble">{img.description}</div>)}
              </div>
            ))
          )}
        </div>
        ...
        <div className="mi-pagination">
          <img src={`${process.env.PUBLIC_URL}/images/LeftArrowButton.png`} alt="Prev" className="mi-prev-btn" style={{ opacity: hasPrev ? 1 : 0.3 }} onClick={() => hasPrev && setCriteria((c) => ({ ...c, page: c.page - 1 }))} />
          <span>Page {criteria.page + 1} / {totalPages}</span>
          <img src={`${process.env.PUBLIC_URL}/images/RightArrowButton.png`} alt="Next" className="mi-next-btn" style={{ opacity: hasNext ? 1 : 0.3 }} onClick={() => hasNext && setCriteria((c) => ({ ...c, page: c.page + 1 }))} />
        </div>

        {selectedImage && (
          <div className="mi-modal-overlay" onClick={() => { setSelectedImage(null); setSelectedImageId(null); setViewSection(null); }}>
            <div className="mi-modal" onClick={(e) => e.stopPropagation()} >
              {viewSection === null ? (
                <span className="mi-action-wrapper" data-tip={viewSection === null ? 'Close' : 'Back'}>
                  <img src={`${process.env.PUBLIC_URL}/images/ExitButton2.png`} alt={viewSection === null ? 'Close' : 'Back'} className="mi-modal-close-btn" onClick={async () => {
                      if (viewSection === null) { await updateImageOnServer(); setSelectedImage(null); setSelectedImageId(null); }
                      setViewSection(null);
                    }}
                  />
                </span>
              ) : (
                <img src={`${process.env.PUBLIC_URL}/images/ExitButton2.png`} alt="Back" className="mi-modal-close-btn" onClick={() => setViewSection(null)} />
              )}

              <div className="mi-action-bar">
                <span className="tooltip tooltip-down mi-action-wrapper"><img src={`${process.env.PUBLIC_URL}/images/PublishButton.png`} alt="Publish" className="mi-icon-button" onClick={handlePublishImage} /><div className="tooltip-bubble">Publish</div></span>
              ...
              {viewSection === null ? (
                <>
                  <div className="mi-tab-group">
                    <button className="mi-tab-btn" onClick={() => setViewSection("sets")}>Sets</button>
                    <button className="mi-tab-btn" onClick={() => setViewSection("params")}>Parameters</button>
                    <button className="mi-tab-btn" onClick={() => setViewSection("constraints")}>Constraints</button>
                    <button className="mi-tab-btn" onClick={() => setViewSection("preferences")}>Preferences</button>
                  </div>
                  <div className="mi-modal-desc"><h2 className="mi-modal-title">{selectedImage.name}</h2><p>{selectedImage.description}</p></div>
                </>
              ) : viewSection === "sets" ? (
                <div className="mi-sets-panel">
                  <div className="mi-sets-controls">
                    <select className="mi-sets-dropdown" value={selectedSetIndex} onChange={(e) => setSelectedSetIndex(Number(e.target.value))}>
                      {selectedImage.sets.map((s, i) => (<option key={i} value={i}>{s.setDefinition.alias}</option>))}
                    </select>
                    <button className="mi-sets-add-btn" onClick={() => { setEditingRow(-1); setEditTupleValues(selectedImage.sets[selectedSetIndex].setDefinition.structure.map(() => "")); }}>Add New Value</button>
                  </div>
                      ...
                      <tr>
                        {selectedImage.sets[selectedSetIndex].setDefinition.structure.map((col, ci) => (
                          <th key={ci}>
                            ...
                            <button className="mi-sort-btn" onClick={() => { setSortConfig((prev) => {
                                if (prev.colIndex !== ci) return { colIndex: ci, direction: "asc" };
                                if (prev.direction === "asc") return { colIndex: ci, direction: "desc" };
                                return { colIndex: null, direction: "asc" };
                              }); }}>
                              {sortConfig.colIndex === ci ? (sortConfig.direction === "asc" ? "▲" : "▼") : "⇅"}
                            </button>
                          </th>
                        ))}
                        <th>Actions</th>
                    ...
                    <tbody>
                      {(() => {
                          ...
                          renderRows.push(
                            <tr key="new">
                              {editTupleValues.map((val, ci) => (
                                <td key={ci}><input className="mi-tuple-input" value={val} onChange={(e) => {
                              ...
                              <td>
                                <button onClick={() => {
                                  const newParts = editTupleValues.map((value) => {
                        ...
                        rows.forEach((row, ri) => {
                          ...
                          renderRows.push(
                            <tr key={ri}>
                              {row.map((cell, ci) => (<td key={ci}>{isEditing ? (<input className="mi-tuple-input" value={editTupleValues[ci] ?? ""} onChange={(e) => {
                                          const c = [...editTupleValues];
                                          c[ci] = e.target.value;
                                          setEditTupleValues(c);
                                        }}/>) : (cell)}</td>))}
                              <td>
                                {isEditing ? (
                                  <>
                                    <button
                                      onClick={() => {
                                        const finalParts = editTupleValues.map((newValue, index) => {
                                          const originalPart = originalEditParts[index] || "";
                                        ...
                                        const newString = `<${finalParts.join(",")}>`;
                                        const img = { ...selectedImage };
                                        
                                        const originalString = `<${originalEditParts.join(",")}>`;
                                        const originalIndex = vals.indexOf(originalString);

                                        if (originalIndex !== -1) {
                                            img.sets[selectedSetIndex].values[originalIndex] = newString;
                                        }
                                  ...
                                  <>
                                    <button
                                      onClick={() => {
                                        const originalStringValue = vals.find(val => {
                                            const cleanVal = val.slice(1, -1).split(',').map(c => c.trim().replace(/^"|"$/g, ''));
                                            return JSON.stringify(cleanVal) === JSON.stringify(row);
                                        });

                                        if (originalStringValue) {
                                            const parts = originalStringValue.slice(1, -1).split(',').map((p) => p.trim());
                                            setOriginalEditParts(parts);
                                            setEditTupleValues(row);
                                            setEditingRow(ri);
                                        }
                                      }}
                                    >✎</button>
                                    <button
                                      onClick={() => {
                                        ...
                                        const indexToRemove = vals.indexOf(originalStringValue);
                                        
                                        if (indexToRemove !== -1) {
                                            img.sets[selectedSetIndex].values.splice(indexToRemove, 1);
                                            setSelectedImage(img);
                                        }
                                      }}
                                    >🗑</button>
                ...
                <div className="mi-params-panel">
                  <table className="mi-params-table">
                    <thead><tr><th>Edit</th><th>Parameter Name</th><th>Value</th></tr></thead>
                    <tbody>
                      {selectedImage.parameters.map((p, i) => (
                        <tr key={i}>
                          <td className="mi-param-cell-edit">
                            {p.isEditing ? (
                              <>
                                <button className="mi-param-btn" onClick={() => {
                                  const img = { ...selectedImage };
                          ...
                          <td className="mi-param-cell-value">
                            {p.isEditing ? (
                              <input type="text" className="mi-param-input" value={p.tempValue} onChange={(e) => {
                                  const img = { ...selectedImage };
                  ...
                  {selectedImage.constraintModules.map((m, idx) => (
                    <div key={idx} className="mi-module-box">
                      <div className="mi-header-row">
                        ...
                        <label className="mi-module-checkbox"><input type="checkbox" checked={m.enabled ?? true} onChange={(e) => {
                              const img = { ...selectedImage };
                ...
                <div className="mi-prefs-panel">
                  {selectedImage.preferenceModules.length === 0 ? (<p>No preferences available.</p>) : (
                    selectedImage.preferenceModules.map((m, idx) => (
                      <div key={idx} className="mi-module-box">
                        <div className="mi-header-row">
                          <div className="mi-module-title">{m.moduleName}</div>
                          <div className="mi-slider-info">
                            <span className="mi-info-icon">?<div className="mi-info-tooltip">Adjusts how strongly this preference is applied (0–100).</div></span>
                            <label htmlFor={`slider-${idx}`}>Value:</label>
                            <input id={`slider-${idx}`} type="range" min="0" max="100" value={m.value ?? 100} onChange={(e) => {
                                const img = { ...selectedImage };
