...
const ConfigureConstraintsPage = () => {
  ...
  const deleteConstraintModule = (index) => {
    setConstraintModules((prevModules) => prevModules.filter((_, i) => i !== index));
    setSelectedModuleIndex(null); // Deselect module after deletion
  };
            ...
            <ul>
              {constraintModules.map((module, index) => (
                <li key={index}>
                  ...
                  <button onClick={() => deleteConstraintModule(index)}>Delete</button> 
                </li>
              ))}
            </ul>
          </div>
